...
# Extract Hero (from <div class="max-w-[1360px]... down to before <!-- Visual Dashboard Grid -->)
...
# Extract Ajustes header
ajustes_header_match = re.search(r'(<div class="flex flex-col">\s*<span class="font-label-sm uppercase tracking-wider text-on-surface-variant">Variables Clave</span>.*?)<div class="grid grid-cols-1 sm:grid-cols-2', html, re.DOTALL)
ajustes_header = ajustes_header_match.group(1) if ajustes_header_match else ""

# Extract Ingesta header
ingesta_header_match = re.search(r'(<div class="flex flex-col">\s*<span class="font-label-sm uppercase tracking-wider text-on-surface-variant">Importación asistida</span>.*?)<!-- Switch Tabs -->', html, re.DOTALL)
ingesta_header = ingesta_header_match.group(1) if ingesta_header_match else ""
...
<script>
    if (!window.parent.document.getElementById('tailwind-script')) {{
        ...
        const fonts2 = window.parent.document.createElement('link');
        fonts2.rel = 'stylesheet';
        fonts2.href = 'https://fonts.googleapis.com/css2?family=Inter:wght@100..900&display=swap';
        window.parent.document.head.appendChild(fonts2);
    }}
</script>
""", height=0, width=0)
...
st.markdown("""
<style>
    .block-container {{ max-width: 1360px; padding-top: 5rem; padding-bottom: 2rem; }}
    ...
    .stTabs [data-baseweb="tab-list"] {{ gap: 24px; border-bottom: 1px solid #E2E8F0; padding-bottom: 8px; }}
    .stTabs [data-baseweb="tab"] {{ font-weight: 500; color: #64748B; font-size: 1rem; }}
...
with col_right:
    if procesar:
        if not uploaded_files and not pasted_text.strip():
            ...
            # Start rendering the right column
            out = f"""
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-space-sm mb-4">
                ...
                <div><span class="px-space-md py-1 rounded-full bg-surface-container-high text-on-surface font-label-sm text-label-sm" style="background:#e9e7ed; padding:4px 16px; border-radius:99px; font-size:11px; font-weight:600;">{len([v for v in resultados.values() if v > 0])} gastos reconocidos</span></div>
            </div>
                    ...
                    <div class="flex items-center justify-between p-space-md rounded-DEFAULT hover:bg-surface-container-low transition-colors" style="padding:16px; border-radius:16px; transition:0.2s;">
                        <div class="flex items-center gap-space-md min-w-0" style="gap:16px;">
                            <div class="w-10 h-10 rounded-DEFAULT bg-surface-container-low flex items-center justify-center text-on-surface-variant shrink-0" style="width:40px; height:40px; border-radius:12px; background:#f4f3f8; color:#4c4546;">
                                <span class="material-symbols-outlined text-[20px]">{get_icon(k)}</span>
                            </div>
                            <div class="flex flex-col min-w-0">
                                ...
                                <div class="flex items-center gap-space-xs" style="gap:4px; font-size:13px; color:#4c4546;">
                                    <span>Detectado aut.</span><span>•</span><span>{fechas.get(k, 'Mes actual')}</span>
                                </div>
                            </div>
                        </div>
                        <div class="text-right">
                            <span class="font-label-lg text-label-lg text-on-surface tabular-nums" style="font-size:15px; font-weight:500; color:#1a1b1f;">$ {int(v):,}</span>
                        </div>
                    </div>
                    """.replace(",", ".")
            ...
            if beneficio_val > 0 and resultados.get("CSFJ (Mensualidad)", 0) > 0:
                ...
                <div class="flex items-center justify-between p-space-md rounded-DEFAULT bg-surface-container-low/60 hover:bg-surface-container-low transition-colors" style="padding:16px; border-radius:16px; background:rgba(218,226,255,0.4);">
                    ...
                    <div class="text-right">
                        <span class="font-label-lg text-label-lg text-secondary tabular-nums" style="font-size:15px; font-weight:500; color:#0053cf;">-$ {int(beneficio_val):,}</span>
                    </div>
                </div>
                """.replace(",", ".")
            ...
            <div class="p-space-lg rounded-DEFAULT bg-surface-container-low flex flex-col sm:flex-row items-start sm:items-center justify-between gap-space-md mt-6" style="padding:24px; border-radius:16px; background-color: rgb(248, 250, 252); border: 1px solid rgb(226, 232, 240);">
                ...
                <div class="flex items-baseline gap-space-xs" style="gap:8px; align-items:baseline;">
                    <span class="font-currency-display text-currency-display text-on-surface tabular-nums leading-none" style="font-size:44px; font-weight:600; color:#1a1b1f; letter-spacing:-0.03em;">$ {int(total):,}</span>
            ...
            <div class="flex flex-col gap-space-md pt-space-xs mt-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-space-md" style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
                    <div class="p-space-lg rounded-DEFAULT bg-secondary-fixed/40 flex flex-col justify-between gap-space-md" style="padding:24px; border-radius:16px; background-color: rgb(239, 246, 255); border: 1px solid rgb(191, 219, 254);">
                        ...
                        <div class="font-headline-lg text-headline-lg text-on-surface tabular-nums" style="font-size:32px; font-weight:600; color:#1a1b1f;">$ {papa:,}</div>
                    </div>
                    ...
                    <div class="p-space-lg rounded-DEFAULT bg-tertiary-fixed/40 flex flex-col justify-between gap-space-md" style="padding:24px; border-radius:16px; background-color: rgb(250, 245, 255); border: 1px solid rgb(233, 213, 255);">
                        ...
                        <div class="font-headline-lg text-headline-lg text-on-surface tabular-nums" style="font-size:32px; font-weight:600; color:#1a1b1f;">$ {mama:,}</div>
                    </div>
                </div>
            </div>
            </section>
