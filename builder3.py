...
navbar_html = navbar_html.replace('href="#"', 'href="javascript:void(0)"')
...
<script>
    if (!window.parent.document.getElementById('tailwind-script')) {
        ...
        config.innerHTML = `tailwind.config = { corePlugins: { preflight: false }, ...<CONFIG> }`;
        ...
        const inter = window.parent.document.createElement('link');
        inter.rel = 'stylesheet';
        inter.href = 'https://fonts.googleapis.com/css2?family=Inter:wght@100..900&display=swap';
        window.parent.document.head.appendChild(inter);
    }
</script>
''', height=0, width=0)
...
st.markdown('''
<style>
    /* Full Reset matching Tailwind exactly */
    .block-container { max-width: 1360px !important; padding-top: 6rem !important; padding-bottom: 2rem !important; }
...
with col_left:
    st.markdown('<section class="bg-surface-container-lowest p-space-xl rounded-[24px] shadow-sm flex flex-col gap-space-lg" style="background-color: rgb(255, 255, 255); border: 1px solid rgb(226, 232, 240); box-shadow: rgba(15, 23, 42, 0.04) 0px 4px 16px -2px;">', unsafe_allow_html=True)
    st.markdown('''<AJUSTES>''', unsafe_allow_html=True)
    ...
    st.markdown('<section class="bg-surface-container-lowest p-space-xl rounded-[24px] shadow-sm flex flex-col gap-space-lg" style="background-color: rgb(255, 255, 255); border: 1px solid rgb(226, 232, 240); box-shadow: rgba(15, 23, 42, 0.04) 0px 4px 16px -2px;">', unsafe_allow_html=True)
    st.markdown('''<INGESTA>''', unsafe_allow_html=True)
...
with col_right:
    if procesar:
        if not uploaded_files and not pasted_text.strip():
            ...
            # Start rendering the right column with exact classes and structure
            out = '''
            <section class="bg-surface-container-lowest p-space-xl rounded-[24px] shadow-sm flex flex-col gap-space-lg" style="background-color: rgb(255, 255, 255); border: 1px solid rgb(226, 232, 240); box-shadow: rgba(15, 23, 42, 0.04) 0px 4px 16px -2px;">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-space-sm mb-4">
                <div class="flex flex-col">
                    ...
                    <h2 class="font-headline-md text-headline-md text-on-surface mt-0.5" style="font-size:24px; font-weight:600; margin-top:4px; letter-spacing:-0.02em;">Desglose Detectado</h2>
            ...
            out += f'''
            <div class="p-space-lg rounded-[16px] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-space-md mt-6" style="padding:24px; border-radius:16px; background-color: rgb(248, 250, 252); border: 1px solid rgb(226, 232, 240);">
                <div class="flex flex-col">
                ...
                <div class="flex items-baseline gap-space-xs" style="gap:8px; align-items:baseline;">
                    <span class="font-currency-display text-currency-display text-on-surface tabular-nums leading-none" style="font-size:44px; font-weight:700; color:#1a1b1f; letter-spacing:-0.03em;">$ {t_val}</span>
                    <span class="font-label-sm text-label-sm text-on-surface-variant" style="font-size:14px; font-weight:600; color:#4c4546;">CLP</span>
                </div>
            </div>
            '''
            ...
            <div class="flex flex-col gap-space-md pt-space-xs mt-8">
                <div class="flex items-center justify-between">
                    <div class="flex flex-col">
                        ...
                        <span class="font-headline-sm text-headline-sm text-on-surface" style="font-size:19px; font-weight:600; color:#1a1b1f; letter-spacing:-0.02em;">Reparto Proporcional Acordado</span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-space-md" style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
                    <div class="p-space-lg rounded-[20px] flex flex-col justify-between gap-space-md" style="padding:24px; border-radius:20px; background-color: rgb(239, 246, 255); border: 1px solid rgb(191, 219, 254);">
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-space-sm" style="gap:12px;">
                                <div class="w-10 h-10 rounded-full flex items-center justify-center font-label-lg font-bold" style="width:40px; height:40px; border-radius:50%; background:#0053cf; color:white; font-weight:bold;">P</div>
                                <div class="flex flex-col">
                                    <span class="font-headline-sm text-headline-sm" style="font-size:15px; font-weight:700; color:#001848;">Cuota Papá</span>
                                </div>
                            </div>
                            <span class="px-space-sm py-1 rounded-full font-label-sm text-label-sm font-semibold" style="padding:4px 8px; border-radius:99px; background:#0053cf; color:white; font-size:11px; font-weight:700;">63,77%</span>
                        </div>
                        <div class="font-headline-lg text-headline-lg tabular-nums" style="font-size:32px; font-weight:700; color:#1a1b1f; letter-spacing:-0.03em;">$ {p_val}</div>
                    </div>
                    
                    <div class="p-space-lg rounded-[20px] flex flex-col justify-between gap-space-md" style="padding:24px; border-radius:20px; background-color: rgb(250, 245, 255); border: 1px solid rgb(233, 213, 255);">
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-space-sm" style="gap:12px;">
                                <div class="w-10 h-10 rounded-full flex items-center justify-center font-label-lg font-bold" style="width:40px; height:40px; border-radius:50%; background:#23005c; color:#e9ddff; font-weight:bold;">M</div>
                                <div class="flex flex-col">
                                    <span class="font-headline-sm text-headline-sm" style="font-size:15px; font-weight:700; color:#23005c;">Deuda de Mamá</span>
                                </div>
                            </div>
                            <span class="px-space-sm py-1 rounded-full font-label-sm text-label-sm font-semibold" style="padding:4px 8px; border-radius:99px; background:#23005c; color:#e9ddff; font-size:11px; font-weight:700;">36,23%</span>
                        </div>
                        <div class="font-headline-lg text-headline-lg tabular-nums" style="font-size:32px; font-weight:700; color:#1a1b1f; letter-spacing:-0.03em;">$ {m_val}</div>
                    </div>
                </div>
            </div>
            </section>
...
app_content = app_content.replace("<CONFIG>", config_json)
app_content = app_content.replace("<NAVBAR>", navbar_html)
app_content = app_content.replace("<HERO>", hero_html)
app_content = app_content.replace("<AJUSTES>", ajustes_header)
app_content = app_content.replace("<INGESTA>", ingesta_header)

with open("app.py", "w") as f:
    f.write(app_content)

print("App rewritten successfully!")
