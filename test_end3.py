...
def get_best(line):
    # Remove quotas ONLY IF they are at the end! 
    # Actually, if we just remove quotas everywhere it's fine, because they don't contain colons.
    line = re.sub(r'\b\d+\s+de\s+\d+\b', ' ', line, flags=re.IGNORECASE)
    line = re.sub(r'\b\d{1,2}/\d{1,2}\b', ' ', line)
    
    # We DO NOT remove times.
    
    # Extract ALL numbers for fallback
